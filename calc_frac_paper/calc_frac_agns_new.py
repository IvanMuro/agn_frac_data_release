...
def _calc_num_agn(
    cube, mask_xrlumi, mask_spatial, mask_selection_gal, mask_gal_agn=True
):
...
def _create_df_resample(mask_xrlumi_dict, mag_lim=False, sm_lim=False):
    ...
    for se, lx in product(se_labels, mask_xrlumi_dict.keys()):
        columns[f"num_agn_{se}_{lx}"] = "i8"

    columns["num_gal_mag"] = "i8"
    columns["num_gal_sm"] = "i8"
...
def _calc_num_gal_agn_lc_catalogue(df_num_gal_agn, lc, id_cluster_resed, m_cut, band,
    ...
    mask_aux = {f"{key}": lc[f"lgLx{key}"] > lx_lim for key in mask_xrlumi_dict}
...
def _get_resample_reseed_sum(df_resed_resample, mask_xrlumi_dict, lgM_smCut, mag_cut):
    ...
    (n_gal_realizations_mag, n_gal_realizations_sm,n_agn_realizations_mag, n_agn_realizations_sm,) = get_resample_agns(df_resed_resample,
       [mask_xrlumi_dict],num_realizations="default", mag_lim=mag_cut, m_lim=lgM_smCut, id_label="id_cluster_resed",)
...
def _add_realization_reseed_sum(
    df_resample_sum,
    ngal,
    n_gal_realizations_sm,
    n_agn_realizations_mag,
    ...
    """ """
    num_agn_mag = [
        sum(n_agn_realizations_mag[f"n_agn_mag_{mask_xrlumi}"][loop_realiz])
        for mask_xrlumi in mask_xrlumi_dict
    ]

    data = copy.deepcopy(num_agn_mag)
    num_agn_sm = [
        sum(n_agn_realizations_sm[f"n_agn_sm_{mask_xrlumi}"][loop_realiz])
        for mask_xrlumi in mask_xrlumi_dict
    ]
