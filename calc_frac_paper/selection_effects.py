...
def get_mask_distFace(dist_spec, cube):
    ...
    mask_x = np.logical_and(np.array(cube  ['x'] > min_dist_face),
                            np.array(cube  ['x'] < max_dist_face))
    mask_y = np.logical_and(np.array(cube  ['y'] > min_dist_face),
                            np.array(cube  ['y'] < max_dist_face))
    mask_xy = np.logical_and(mask_x, mask_y)
    mask_z = np.logical_and(np.array(cube  ['z'] > min_dist_face),
                            np.array(cube  ['z'] < max_dist_face))
...
def get_spatial_mask(id_cluster, ang, lc, delta_z, field=False, ang_max=6/60, r200=False):
    """
...
def get_mask_spec(lc):
    ...
    """
    mask_spec = np.array( np.abs( lc  ['final_redshift'] - 
                                  lc.observation['desire_redshift'] ) < delta_z_spec_crit(lc) )
    return mask_spec
...
def delta_z_spec_crit(lc, cube=False):
    if cube == False:
        if lc.z > 1:
            ...
            lgm_clust_200 = np.log10(mass_defs.changeMassDefinition(m_clust, c, lc.z, 'vir', '200c')[0])
...
def get_mask_ang_field(cube, ang_min, ang_max, x_clust, y_clust):
    ...
    mask_ang_min = np.array(np.sqrt((cube  ['x']-x_clust)**2 + 
                           (cube  ['y']-y_clust)**2) > dist_ang_min)
    mask_ang_max = np.array(np.sqrt((cube  ['x']-x_clust)**2 + 
                           (cube  ['y']-y_clust)**2) < dist_ang_max)
...
def _add_m200(cube):
    ...
    """
    c = 10**.7 #Fig.1 Ludlow+14
    for mass in ['lgmp', 'lgm']:
        mass200m = mass_defs.changeMassDefinition(10**cube  [mass], c, cube.z, 'vir', '200m')
...
def _calc_angle_from_r(lc):
    ...
    d_ang_cosmo = lc.cosmo.angularDiameterDistance(lc.observation['desire_redshift']) * (u.Mpc/u.rad)
